...
export const getPendingHoldMinutes = (): number =>
  parsePositiveInt(process.env.BOOKING_PENDING_HOLD_MINUTES, 15);
export const WORK_DAY_START_MINUTES = 9 * 60;
export const WORK_DAY_END_MINUTES = 18 * 60;
export const SLOT_INTERVAL_MINUTES = 30;

export const activeAppointmentStatusSql = (
  paramRef: string,
  alias = "",
): string => {
...
const timeStringToParts = (time: string): TimeParts | null => {
...
import { ValidateBookableSlotInput, ValidationResult } from "../types";

export const validateBookableSlot = ({
  appointmentDate,
  startTime,
  durationMins,
  now = new Date(),
}: ValidateBookableSlotInput): ValidationResult => {
  ...
  if (timeParts.seconds !== 0) {
    return {
      valid: false,
      error: "Appointment start time must use a 30-minute slot",
    };
  }
