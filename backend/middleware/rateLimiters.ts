...
const buildRateLimitHandler = (message: string) => (req: Request, res: Response) => {
    res.status(429).json({
...
}: CreateRateLimiterInput) => rateLimit({
    windowMs,
    limit,
...
export const generalApiLimiterFallback = (req: Request, res: Response, next: NextFunction) => {
