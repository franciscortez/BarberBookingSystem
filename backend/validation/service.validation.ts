...
const decimalField = (label: string) =>
  z
    .union([
      ...
      z
        .string()
        .regex(/^\d+(\.\d{1,2})?$/, `${label} must be a valid decimal number`),
    ])
...
export const CreateServiceSchema = z
  ...
  .refine(
    (data) =>
      parseFloat(data.downpayment_amount) <= parseFloat(data.total_price),
    {
      message: "downpayment_amount must not exceed total_price",
