...
describe("Availability API Endpoints", () => {
  ...
  afterAll(async () => {
    ...
    await pool.query("DELETE FROM Appointments WHERE barber_id = $1", [
      barberId,
    ]);
  ...
  it("GET /api/availability should return all slots when no appointments exist", async () => {
    ...
    expect(res.body.slots.length).toBeGreaterThanOrEqual(
      res.body.availableSlots.length,
    );
  ...
  it("GET /api/availability should exclude occupied slots", async () => {
    ...
    const isTenAvailable = res.body.availableSlots.some(
      (slot: any) => slot.start === "10:00",
    );
    const isTenThirtyAvailable = res.body.availableSlots.some(
      (slot: any) => slot.start === "10:30",
    );
    const isNineAvailable = res.body.availableSlots.some(
      (slot: any) => slot.start === "09:00",
    );
  ...
  it("GET /api/availability should ignore expired pending appointments", async () => {
    ...
    expect(res.statusCode).toEqual(200);
    expect(
      res.body.availableSlots.some((slot: any) => slot.start === "12:00"),
    ).toBe(true);
    expect(
      res.body.slots.find((slot: any) => slot.start === "12:00").available,
    ).toBe(true);
  });
  ...
  it("GET /api/availability should keep fresh pending appointments unavailable", async () => {
    ...
    expect(res.statusCode).toEqual(200);
    expect(
      res.body.availableSlots.some((slot: any) => slot.start === "13:00"),
    ).toBe(false);
    expect(
      res.body.slots.find((slot: any) => slot.start === "13:00").available,
    ).toBe(false);
  });
