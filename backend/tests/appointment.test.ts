...
const mockSendCancellationConfirmationEmail = sendCancellationConfirmationEmail as jest.Mock;
...
describe('Appointment & Payment Integration API Endpoints', () => {
    ...
    it('POST /api/appointments should create a booking and initialize checkout', async () => {
        ...
        const checkoutRequest = JSON.parse(((global as any).fetch as jest.Mock).mock.calls[0][1].body);
    ...
    it('POST /api/payments/webhook should process successful payment event', async () => {
        ...
        const signature = crypto.createHmac('sha256', webhookSecret as string).update(payloadStr).digest('hex');
        ...
        expect(mockSendConfirmationEmail).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('POST /api/payments/webhook should ignore duplicate successful payment events', async () => {
        ...
        const payloadBody = JSON.stringify(webhookPayload);
        const payloadStr = timestamp + '.' + payloadBody;
        const signature = crypto.createHmac('sha256', paymongoConfig.webhookSecret as string).update(payloadStr).digest('hex');
    ...
    it('POST /api/payments/webhook should still succeed if confirmation email fails', async () => {
        ...
        mockSendConfirmationEmail.mockRejectedValueOnce(new Error('SMTP unavailable'));
    ...
    it('POST /api/payments/webhook should not partially update appointment state if payment update fails', async () => {
        ...
        const updateSpy = jest.spyOn(Payment, 'updatePayment').mockRejectedValueOnce(new Error('simulated payment update failure') as never);
    ...
    it('POST /api/appointments/reschedule should still succeed if email fails', async () => {
        ...
        mockSendRescheduleConfirmationEmail.mockRejectedValueOnce(new Error('SMTP unavailable'));
    ...
    it('POST /api/appointments/cancel should still succeed if email fails', async () => {
        ...
        mockSendCancellationConfirmationEmail.mockRejectedValueOnce(new Error('SMTP unavailable'));
