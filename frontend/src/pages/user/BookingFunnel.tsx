  ...
  useEffect(() => {
    ...
    const fetchCatalog = async () => {
      try {
        ...
        setError(
          "Unable to load the booking catalog. Please refresh the page.",
        );
      } finally {
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans pb-16 selection:bg-amber-500/30 selection:text-amber-200 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-24 sm:pt-28">
        ...
        <p className="text-zinc-400 text-sm sm:text-base mt-1">
          Reserve your customized grooming session with simple, secure steps.
        </p>
      </div>

      <div className="max-w-4xl mx-auto px-4 sm:px-6 mt-6 sm:mt-8">
        {/* Step Progress Bar */}
        <div className="relative rounded-2xl border border-zinc-800/80 bg-zinc-900/20 backdrop-blur-md p-4 sm:p-6 mb-6 sm:mb-8 overflow-hidden">
            ...
            {steps.map((s, idx) => {
              ...
              return (
                ...
                >
                  {idx < steps.length - 1 && (
                    <div
                      className={`absolute top-5 left-1/2 right-[-50%] h-[2px] -translate-y-1/2 z-[-1] transition-all duration-500 ${isCompleted ? "bg-amber-500" : "bg-zinc-800"}`}
                    />
                  )}
                  <div
                  ...
                  <span
                    className={`mt-3 text-xs font-medium tracking-wide text-center hidden md:block transition-colors duration-300 ${
                      isActive
                        ? "text-amber-400"
                        : isCompleted
                          ? "text-zinc-300"
        ...
        {/* Step Content */}
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/30 backdrop-blur-md p-4 sm:p-8 min-h-[400px] flex flex-col justify-between">
          {currentStep === 1 && (
            <BarberServiceStep
              barbers={barbers}
              services={services}
